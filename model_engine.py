...
class EduGuardianModel:
    def __init__(self):
        # Using RandomForest Ensemble: High accuracy and provides internal variability for confidence scoring
    ...
    def _train_realistic_data(self):
        # Creating a realistic synthetic dataset (500 samples for technical depth)
        np.random.seed(42)
        n_samples = 500
        X = np.column_stack([
            np.random.uniform(30, 100, n_samples),  # Attendance
            np.random.uniform(10, 100, n_samples),  # Marks
            np.random.uniform(1, 5, n_samples),     # Wellness
            np.random.randint(0, 2, n_samples),     # Digital Access
            np.random.uniform(0.5, 25, n_samples),  # Travel KM
            np.random.randint(1, 4, n_samples),     # Income (1:Low, 2:Mid, 3:High)
    ...
    def predict_pro(self, data):
        ...
        # WOW FEATURE: Ensemble Confidence Score
        # Calculates how 'sure' the AI is by checking consistency across its 100 internal trees
        ...
        # Actionable AI: Predictive Timeline (Risk decay over 6 months)
        timeline = [round(prob * (0.88 ** i), 2) for i in range(6)]
        
        # XAI: Explainable AI - Identify the Top Socio-Academic Risk Driver
